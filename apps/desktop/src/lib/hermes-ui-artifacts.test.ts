...
const canonicalApproval = {
  ...
  operations: [
    { clientId: 'draft', doneEnough: 'One page exists', kind: 'create', order: 1, title: 'Draft brief' }
  ],
...
describe('parseHermesUiArtifact', () => {
  ...
  it('rejects unknown envelope fields, unstable step identities, and out-of-bounds edits', () => {
    ...
    expect(parseHermesUiArtifact(JSON.stringify({
      ...breakdown,
      steps: [{ clientId: 'same', doneEnough: 'A', title: 'A' }, { clientId: 'same', doneEnough: 'B', title: 'B' }]
    })).ok).toBe(false)
    expect(parseHermesUiArtifact(JSON.stringify({
      ...breakdown,
  ...
  it('allows empty editable draft text but never unsupported persisted fields', () => {
    ...
    ])).toEqual([{ clientId: 'new', doneEnough: '', title: '' }])
    expect(parseHermesUiTaskBreakdownDraftSteps([
      { clientId: 'new', command: 'rm', doneEnough: 'Done', title: 'Unsafe' }
    ])).toBeNull()
  })

  it('keeps identities that normalize alike in separate draft storage slots', () => {
    const first = parseHermesUiArtifact(JSON.stringify({
      ...breakdown,
      task: { ...breakdown.task, id: 'task/a' }
    }))

    const second = parseHermesUiArtifact(JSON.stringify({
      ...breakdown,
      task: { ...breakdown.task, id: 'task a' }
    }))
  ...
  it('parses exact canonical mutation proof and keeps operation order', () => {
    const result = parseHermesUiArtifact(JSON.stringify({
      canonicalApproval,
      changes: [{ after: { steps: 2 }, before: { steps: 1 }, operation: 'update', taskId: 'task-42', title: 'Prepare launch brief' }],
      title: 'Approve exact breakdown',
      type: 'mutation-preview'
    }))

    expect(result.ok).toBe(true)
    expect(result.ok && result.artifact.type === 'mutation-preview' && result.artifact.canonicalApproval.operations[0]).toMatchObject({
      clientId: 'draft',
      kind: 'create',
      order: 1
    })
