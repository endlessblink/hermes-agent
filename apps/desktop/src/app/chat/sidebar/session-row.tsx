...
export function SidebarSessionRow({
  ...
  // Subscribe per-row (the leaf) instead of drilling sets through the list —
  // the atoms are tiny and rarely non-empty.
  // True when a clarify prompt in this session is waiting on the user.
  const needsInput = useStore($attentionSessionIds).includes(session.id)
  // True when this session's most recent turn finished and is awaiting the user
  // — renders the steady "1" reply-ready badge on top of the dot state.
        ...
        >
          {reorderable ? (
            ...
            <SidebarRowLead className={needsInput ? 'overflow-visible' : 'overflow-hidden'}>
              <SessionRowLeadDot branchStem={branchStem} dotState={dotState} replyReady={replyReady} />
            </SidebarRowLead>
          )}
...
function SessionRowLeadDot({
  ...
  return (
    <span className={cn('flex items-center gap-0.5', className)}>
      ...
      <SidebarRowDot dotState={dotState} replyReady={replyReady} />
    </span>
  )
}
