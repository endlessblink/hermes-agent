...
def test_monitor_classifier_retries_the_same_deferred_event_instead_of_merging_it(
    tmp_path,
):
    from agent.personal_assistant_state import PersonalAssistantStateStore

    store = PersonalAssistantStateStore(tmp_path)
    deferred = _monitor_event(
        "deferred", task_id="task-active", operation_id="external-change"
    )
    store.merge_monitor_events([deferred], disposition="retry_wait")

    classified = store.classify_monitor_events([deferred])

    assert classified["merged"] == []
    assert [event["id"] for event in classified["remaining"]] == ["deferred"]
