...
class TestPlanToolBatchSegments:
    ...
    def test_full_task_inventory_reads_are_sequential_so_same_turn_deduplication_can_observe_completion(
        self, monkeypatch
    ):
        monkeypatch.setattr(
        ...
        calls = [
            _tc("flowstate_list_tasks", '{"limit":100}', call_id="inventory-all"),
            _tc("flowstate_list_tasks", '{"limit":25}', call_id="inventory-repeat"),
        ]

        segments = _plan_tool_batch_segments(calls)

        assert _kinds(segments) == ["sequential"]
        assert [tc.id for tc in segments[0][1]] == ["inventory-all", "inventory-repeat"]
