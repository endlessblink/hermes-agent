...
def _resolve_path_for_task(filepath: str, task_id: str = "default") -> Path | PurePosixPath:
    ...
    """
    container_paths = _uses_container_paths(task_id)
    # ``~`` resolves against the real OS home, not the profile HOME that
    # profile-scoped Desktop/gateway turns run under: a user writing
    # ``~/.life-os/...`` means their account home. Upstream expands with
    # ``_expand_tilde`` (profile home), which would reroute those paths.
    if container_paths:
    ...
    expanded = os.fspath(_expand_user_path(_msys_to_windows_path(filepath)))
    if sys.platform == "win32":
        import ntpath
