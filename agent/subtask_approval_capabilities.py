"""Ephemeral, session-bound proof that a user approved an exact subtask apply."""
...
_REQUEST_KEYS = _PROOF_KEYS


class ApprovalCapabilityError(ValueError):
    """Raised when an approval capability is absent, stale, or mismatched."""
...
def _parse_expiry(value: Any) -> datetime:
    if (
        ...
        or len(value) > 64
        or not _CANONICAL_TIMESTAMP_RE.fullmatch(value)
    ):
...
def _validate_proof(proof: Any) -> tuple[dict[str, Any], datetime]:
    ...
    identity_limits = {"taskId": 160, "operationId": 160, "proposalId": 120}
    if any(
        not isinstance(proof[key], str)
        or not proof[key]
        or proof[key] != proof[key].strip()
        or len(proof[key]) > limit
    ...
    if not all(isinstance(item, dict) for item in proof["operations"]):
        raise ApprovalCapabilityError("approval proof operations are invalid")
    if any(
...
class SubtaskApprovalCapabilityRegistry:
    """Thread-safe in-memory registry for one-session, one-proof capabilities."""
    ...
    def register(self, session_key: str, proof: dict[str, Any]) -> str:
        ...
        with self._lock:
            ...
            while capability in self._capabilities:
                capability = secrets.token_urlsafe(32)
    ...
    def authorize(
        ...
        with self._lock:
            ...
            expected = {key: record.proof[key] for key in _REQUEST_KEYS}
            if request != expected:
                raise ApprovalCapabilityError("approval capability proof does not match")
            # The first authorization consumes the token for any other request.
            # Exact repeats remain valid so the canonical server operation can
            # be replayed after a response is lost.
    ...
    def invalidate_proposal(
        ...
        with self._lock:
            self._invalidated_revisions[lineage] = max(
                proposal_revision,
                self._invalidated_revisions.get(lineage, 0),
            )
            tokens = [
                token
    ...
    def revoke(self, session_key: str, proof: dict[str, Any]) -> int:
    ...
    def revoke_session(self, session_key: str) -> int:
        with self._lock:
            ...
            for token in tokens:
                self._capabilities.pop(token, None)
            lineages = [lineage for lineage in self._invalidated_revisions if lineage[0] == session_key]
