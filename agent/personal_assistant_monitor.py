...
def classify_connector_failure(exc: BaseException) -> dict[str, str]:
    ...
    if code == "invalid_inventory_receipt":
        return _sanitize_connector_failure(
            {"category": "invalid_response", "code": "invalid_inventory_receipt"}
        )
...
def fetch_flowstate_context(request=None) -> dict[str, Any]:
    from tools.flowstate_tool import _FlowStateApiError, _validated_inventory_receipt
    ...
    inventory = _validated_inventory_receipt(dict(task_payload))
    if inventory.get("complete") is not True:
        raise _FlowStateApiError(
            "Flow State inventory is incomplete; monitor comparison was refused.",
