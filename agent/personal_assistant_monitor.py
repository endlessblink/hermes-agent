...
if sys.platform != "win32":
    import fcntl
else:  # pragma: no cover - exercised through a simulated Windows lock path
    fcntl = None
...
def _locked_file(path: Path) -> Iterator[None]:
    ...
    with _thread_lock(path), path.open("a+b") as handle:
        if sys.platform == "win32":
            import msvcrt
...
def fetch_flowstate_context(request=None) -> dict[str, Any]:
    ...
    inventory = request("GET", "/api/tasks/inventory")
