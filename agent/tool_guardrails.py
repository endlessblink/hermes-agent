...
_TRUSTED_TOOL_HALT_ACTIONS = {
    (
        ...
        "FlowState rejected the merge because the recurrence definitions need an exact user choice. "
    ...
    (
        ...
        "FlowState rejected the merge because one or both tasks have established recurrence history. "
...
_COMPUTER_USE_SURFACE_UNAVAILABLE_MARKERS = (
    ...
    def after_call(
        ...
        *,
        ...
        if isinstance(result_data, dict):
            if tool_name == "computer_use" and result_data.get("ok") is False:
                surface_message = str(
                    result_data.get("message") or result_data.get("error") or ""
                ).lower()
                if any(
                    marker in surface_message
                    for marker in _COMPUTER_USE_SURFACE_UNAVAILABLE_MARKERS
                ):
                    halt_contract = (
                        "computer_use_surface_unavailable",
                        "Computer Use cannot safely send input while the target Electron window "
                        "is hidden or unfocused. Stop retrying this action. Ask the user to bring "
                        "the target window to the foreground, then capture fresh UI state before "
                        "attempting any further input.",
                    )
