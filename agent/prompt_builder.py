...
DESKTOP_QUESTIONNAIRE_GUIDANCE = (
    ...
    "regenerate the preview. Before rendering it for an existing FlowState task, make a fresh `flowstate_list_subtasks` "
    "read and copy its parent canonical revision into `task.baseRevision`. A task-breakdown must use schemaVersion 1, "
    "a stable proposalId, a monotonically increasing proposalRevision, and exactly one stable `subtaskId` or `clientId` "
    "per row. Editing is never approval: a `task-breakdown-revision` response has approval=false and invalidates every "
    "older preview. Preview the exact ordered operations with `flowstate_subtask_batch`; then render a `mutation-preview` "
    "whose `canonicalApproval` copies the tool's verified `approvalRequest` exactly and adds the matching proposalId and "
    "proposalRevision. Do not add model-authored actions to that canonical preview. Apply only after the renderer returns "
    "a `flowstate-mutation-decision` with decision=approve, approval=true, the unchanged proof, and no correction. If the "
    "decision is revise, the proof expired, or the tool returns `stale_revision`, re-read fresh subtasks and generate a new "
    "proposal revision and operation ID. If it returns `invalid_existing_subtasks`, stop instead of retrying or rewriting "
    "the array and report that existing FlowState subtask data needs a bounded repair. Call `flowstate_subtask_batch` only "
    "after the user explicitly approves that exact "
    "preview; never apply an earlier version or infer approval from editing or submitting a revision."
