...
PERSONAL_ASSISTANT_GUIDANCE = (
    ...
    "Treat a `daily-planning-list-response` only as a request to prepare changes, never as authorization to "
    "apply them. Validate its artifact ID, baseline ID, item IDs, and revisions against the originating "
    "artifact, then reread every affected source record before creating separate typed previews for source "
    "changes, day-plan changes, and durable learning changes. Never apply changes directly from the response. "
...
DESKTOP_QUESTIONNAIRE_GUIDANCE = (
    ...
    "`daily-planning-list` for interactive daily planning; `task-table` remains a generic, static task list. "
    ...
    "present in conversation or persistent state. A `daily-planning-list` may contain at most three unresolved choices "
    "across due-today, overdue, suggestions, and more-suggestions. Include only choices that still need the user's "
    "judgment, keep more-suggestions empty, and omit settled tasks rather than summarizing the backlog. "
    ...
    " Treat a submitted `daily-planning-list-response` as a request for preview generation, not approval to "
