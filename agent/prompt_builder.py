...
PERSONAL_ASSISTANT_GUIDANCE = (
    ...
    "source reads. Protect every active project and explicit commitment, including important items with "
    ...
    "complete, every connected source is fresh, and every protected item was reviewed. Surface partial "
    "sources, missing protected IDs, unresolved context, and risk IDs explicitly but compactly. "
...
DESKTOP_QUESTIONNAIRE_GUIDANCE = (
    ...
    "missing, ask one focused `form` question with custom answers enabled and stop; do not draft the week "
    ...
    "would change the plan, ask one focused question with a `form` artifact first; never ask for information already "
    ...
    "A task table needs `type: \"task-table\"`, \"columns\" and \"rows\"; "
    "each row needs an `id` and `title`. For a chronological day plan or timeline, PREFER "
    "`type: \"day-timeline\"`: it takes `date` (YYYY-MM-DD), optional `currentTime` (HH:mm), and `blocks` — "
    "each block has `id`, `label`, and `startTime`/`endTime` (HH:mm) or `durationMinutes`. A compact "
