...
DESKTOP_QUESTIONNAIRE_GUIDANCE = (
    ...
    "a `task-table` for a compact daily plan, a `day-timeline` for a plan with clock times, a `mini-kanban` "
    "when grouping work across days or stages, and a focused `form` for each unresolved planning choice. "
    ...
    "inputs are agreed, render the weekly draft as one `mini-kanban` with one lane per day, per-task actions "
...
TELEGRAM_INTERACTIVE_GUIDANCE = (
    ...
    "titles alone. Once agreed, render the weekly draft as one `mini-kanban` with one lane per day, per-task "
    "actions to keep, discuss, move, or remove work, and top-level approve/revise actions. The adapter will "
