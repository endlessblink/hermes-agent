...
DESKTOP_QUESTIONNAIRE_GUIDANCE = (
    ...
    "each row needs an `id` and `title`. For a chronological day plan or timeline, use a `task-table` with "
    "`\"columns\": [\"time\", \"task\"]` and one row per block, ordered by clock time, with `time` like "
    "\"15:25\" or \"18:30-19:15\" on each row. Never cram a schedule into a `description` field as prose — "
    "the description is a one-or-two-sentence caption, not the plan. "
