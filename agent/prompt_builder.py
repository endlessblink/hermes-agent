...
DESKTOP_QUESTIONNAIRE_GUIDANCE = (
    ...
    "a `week-planner` for every weekly or multi-day plan, a `mini-kanban` only for workflow stages, and a "
    "focused `form` for each unresolved planning choice. Never use `mini-kanban` as a calendar. "
    ...
    "A week planner needs `type: \"week-planner\"`, `weekStart` (YYYY-MM-DD), optional `currentDate`, and exactly "
    "seven consecutive `days`. Each day needs `id`, `label`, `date`, and `blocks`; each block needs `id` and "
    "`label`, plus optional `startTime`, `endTime`, `durationMinutes`, `kind`, `status`, `doneEnough`, `note`, "
    "`confidence`, `taskId`, and submit-only `actions`. Put every useful weekly-plan detail inside those fields. "
