...
class GatewayAuthorizationMixin:
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        def _auth_env(name: str, default: str = "") -> str:
            """Read authorization settings from the active profile scope."""
        ...
        if source.chat_type in {"group", "forum", "channel"} and source.chat_id:
            ...
            if chat_allowlist_env:
                raw_chat_allowlist = _auth_env(chat_allowlist_env).strip()
        ...
        platform_allowlist = _auth_env(platform_env_map.get(source.platform, "")).strip()
        group_user_allowlist = ""
        group_chat_allowlist = ""
        if source.chat_type in {"group", "forum"}:
            group_user_allowlist = _auth_env(platform_group_user_env_map.get(source.platform, "")).strip()
            group_chat_allowlist = _auth_env(platform_group_chat_env_map.get(source.platform, "")).strip()
        global_allowlist = _auth_env("GATEWAY_ALLOWED_USERS").strip()
